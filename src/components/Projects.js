          ...
          {ProjectsData.reverse().map((project) => (
            ...
            >
              <img
                src={project.image}
                alt={project.name}
                className={`h-80 rounded-xl ${project.url ? 'cursor-pointer' : ''}`}
                onClick={() => project.url && window.open(project.url, '_blank')}
              />
              <div 
                className={`absolute flex flex-col justify-center items-center bottom-0 left-0 right-0 h-0 overflow-hidden group-hover:h-full transition-[height_0.5s] duration-500 bg-darkblue rounded-xl ${project.url ? 'cursor-pointer' : ''}`}
                onClick={() => project.url && window.open(project.url, '_blank')}
              >
